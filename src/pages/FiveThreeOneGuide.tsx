...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="rounded-[2px] p-4" style={CARD}>
      <h2 className="text-[15px]" style={{ color: '#e2e2e2' }}>{title}</h2>
      <div className="text-[13px] leading-relaxed mt-2 space-y-2" style={{ color: '#a6a6a6' }}>
        {children}
      </div>
    </section>
  )
...
export default function FiveThreeOneGuide() {
  ...
  return (
    <div className="page-container space-y-4">
      ...
      <div className="pt-1">
        <div className="text-[11px] uppercase tracking-widest" style={{ color: '#a6a6a6' }}>Reference</div>
        <h1 className="text-[20px] uppercase tracking-wide" style={{ color: '#e2e2e2' }}>5/3/1 Guide</h1>
      </div>
      ...
      <Section title="The 3-week wave">
        ...
        <ul className="list-disc pl-5 space-y-1">
          <li><span style={{ color: '#e2e2e2' }}>Week 1 — 5s:</span> 65% / 75% / 85%, for 5/5/5+ reps</li>
          <li><span style={{ color: '#e2e2e2' }}>Week 2 — 3s:</span> 70% / 80% / 90%, for 3/3/3+ reps</li>
          <li><span style={{ color: '#e2e2e2' }}>Week 3 — 5/3/1:</span> 75% / 85% / 95%, for 5/3/1+ reps</li>
        </ul>
      ...
      <Section title="The AMRAP top set">
        <p>
          The final set of each week is marked with a <span style={{ color: '#86efac' }}>+</span> — As Many
      ...
      <Section title="Cycle progression">
        <p>
          ...
          cycle — typically <span style={{ color: '#e2e2e2' }}>+5 lb</span> on upper-body lifts and{' '}
          <span style={{ color: '#e2e2e2' }}>+10 lb</span> on lower-body lifts. Small, steady increases are
