  ...
  return (
    <Modal open={open} onClose={onClose} title="Daily Check-in">
      <div className="space-y-6">
        ...
        {/* Mood */}
        <div>
          <div className="text-[length:var(--text-xs)] uppercase tracking-widest text-noir-muted mb-3">
        ...
        {/* Soreness */}
        <div>
          <div className="text-[length:var(--text-xs)] uppercase tracking-widest text-noir-muted mb-3">Soreness</div>
          <div className="grid grid-cols-4 gap-1.5">
            {(['none', 'mild', 'moderate', 'high'] as Soreness[]).map(level => (
              <button
                ...
                className={cn(
                  'py-2 border rounded-[2px] text-[length:var(--text-xs)] uppercase tracking-widest transition-colors',
                  soreness === level
                    ? level === 'high'
          ...
          <div className="flex gap-2">
            {[false, true].map(val => (
              <button
                ...
                className={cn(
                  'flex-1 py-2 border rounded-[2px] text-[length:var(--text-sm)] uppercase tracking-widest transition-colors',
                  forearmFatigue === val
                    ? val
        ...
        <div>
          ...
          <input
            ...
            className="w-full bg-noir-bg border border-noir-border rounded-[2px] px-3 py-2 text-[length:var(--text-base)] text-noir-accent placeholder:text-noir-dim focus:outline-none focus:border-noir-strong"
          />
          {weightError && (
            <p id="checkin-weight-error" role="alert" className="text-[length:var(--text-sm)] mt-1.5" style={{ color: 'var(--red)' }}>
              {weightError}
            </p>
          )}
        </div>
        ...
        {/* Notes */}
        <div>
          <div className="text-[length:var(--text-xs)] uppercase tracking-widest text-noir-muted mb-2">Notes (optional)</div>
          <textarea
            value={notes}
            onChange={e => setNotes(e.target.value)}
            placeholder="Anything to note..."
            rows={2}
            className="w-full bg-noir-bg border border-noir-border rounded-[2px] px-3 py-2 text-[length:var(--text-md)] text-noir-accent placeholder:text-noir-dim focus:outline-none focus:border-noir-strong resize-none"
          />
        </div>

        <div className="space-y-2">
          {saveError && (
            <p role="alert" className="text-[length:var(--text-md)]" style={{ color: 'var(--red)' }}>{saveError}</p>
          )}
  ...
  return (
    <div>
      <div className="flex gap-1.5">
        {[1, 2, 3, 4, 5].map(n => (
          <button
            ...
            onClick={() => onChange(n)}
            className={cn(
              'flex-1 py-3 border rounded-[2px] text-[length:var(--text-lg)] transition-colors',
              value === n
      ...
      <div className="flex justify-between mt-1">
        <span className="text-[length:var(--text-2xs)] uppercase tracking-widest text-noir-dim">{labels[0]}</span>
        <span className="text-[length:var(--text-2xs)] uppercase tracking-widest text-noir-dim">{labels[4]}</span>
      </div>
    </div>
  )
}
