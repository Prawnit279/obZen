  ...
  return (
    <div>
      ...
      <svg width="100%" viewBox={`0 0 ${W} ${H}`}>
        ...
        <text x={cx} y={cy + 4} style={{ fontSize: 'var(--text-base)' }} fill="var(--muted)" textAnchor="middle">
        ...
        {CATEGORIES.map((cat, i) => {
          ...
          return (
            <g key={cat} transform={`translate(128, ${16 + i * 26})`}>
              ...
              <text x="12" y="8" style={{ fontSize: 'var(--text-sm)' }} fill="var(--accent)">{cat}</text>
              <text x="12" y="18" style={{ fontSize: 'var(--text-xs)' }} fill="var(--dim)">{pct}% · {fmtMin(min)}</text>
            </g>
          )
        })}
      </svg>
