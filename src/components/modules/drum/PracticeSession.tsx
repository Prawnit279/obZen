...
export function PracticeSession({ currentBpm, onComplete }: Props) {
    ...
    return (
      <div className="space-y-4 p-4 border border-[color:var(--violet-400)]/40 rounded-[var(--r-control)] bg-white/[0.05]">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-[length:var(--text-sm)] uppercase tracking-widest text-[color:var(--ink-dim)]">Session in progress</div>
            <div className="flex items-center gap-2 mt-1">
              <Timer size={14} className="text-[color:var(--ink-faint)]" />
              <span className="text-[length:var(--text-7xl)] text-[color:var(--ink)] font-light font-mono leading-none">
                {String(Math.floor(elapsed / 60)).padStart(2, '0')}:{String(elapsed % 60).padStart(2, '0')}
              </span>
            </div>
          </div>
          <div className="text-right">
            <div className="text-[length:var(--text-sm)] text-[color:var(--ink-faint)]">Metronome</div>
            <div className="text-[length:var(--text-4xl)] text-[color:var(--ink-2)]">{currentBpm} BPM</div>
          </div>
        </div>
    ...
    return (
      <div className="space-y-4 p-4 border border-[color:var(--hairline)] rounded-[var(--r-control)]">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[length:var(--text-sm)] uppercase tracking-widest text-[color:var(--ink-2)]">Session Complete</span>
        </div>
        <div className="text-[length:var(--text-base)] text-[color:var(--ink-dim)]">
          Duration: {formatDuration(Math.floor(elapsed / 60))}
          {elapsed % 60 > 0 && ` ${elapsed % 60}s`}
        </div>
        <div>
          <div className="text-[length:var(--text-sm)] uppercase tracking-widest text-[color:var(--ink-faint)] mb-2">BPM Achieved</div>
          <input
            value={bpmAchieved}
            onChange={e => setBpmAchieved(e.target.value.replace(/\D/, ''))}
            placeholder={`${currentBpm} (metronome BPM)`}
            className="w-full bg-[color:var(--bg)] border border-[color:var(--hairline)] rounded-[var(--r-control)] px-3 py-2 text-[length:var(--text-md)] text-[color:var(--ink-2)] placeholder:text-[color:var(--ink-faint)] focus:outline-none focus:border-[color:var(--border-strong)]"
          />
        </div>
        <div>
          <div className="text-[length:var(--text-sm)] uppercase tracking-widest text-[color:var(--ink-faint)] mb-2">Notes (optional)</div>
          <textarea
            value={notes}
            onChange={e => setNotes(e.target.value)}
            placeholder="What you worked on, what clicked, what to revisit..."
            rows={2}
            className="w-full bg-[color:var(--bg)] border border-[color:var(--hairline)] rounded-[var(--r-control)] px-3 py-2 text-[length:var(--text-md)] text-[color:var(--ink-2)] placeholder:text-[color:var(--ink-faint)] focus:outline-none focus:border-[color:var(--border-strong)] resize-none"
          />
        </div>
  ...
  return (
    <div className="space-y-4 p-4 border border-[color:var(--hairline)] rounded-[var(--r-control)]">
      <div className="text-[length:var(--text-sm)] uppercase tracking-widest text-[color:var(--ink-dim)]">New Practice Session</div>
      ...
      <div className="grid grid-cols-3 gap-1.5">
        {(['rudiment', 'lesson', 'free'] as FocusType[]).map(t => (
          <button
            ...
            className={cn(
              'py-2 border rounded-[var(--r-control)] text-[length:var(--text-sm)] uppercase tracking-widest transition-colors',
              focusType === t
      ...
      {focusType === 'rudiment' && (
        <div>
          <div className="text-[length:var(--text-sm)] uppercase tracking-widest text-[color:var(--ink-faint)] mb-2">Select Rudiment</div>
          <select
      ...
      {focusType === 'lesson' && (
        <div>
          <div className="text-[length:var(--text-sm)] uppercase tracking-widest text-[color:var(--ink-faint)] mb-2">Select Lesson</div>
          <select
      ...
      {focusType === 'free' && (
        <div>
          <div className="text-[length:var(--text-sm)] uppercase tracking-widest text-[color:var(--ink-faint)] mb-2">Focus Area</div>
          <div className="flex flex-wrap gap-1.5">
            {FOCUS_AREAS.map(area => (
              <button
                ...
                className={cn(
                  'px-2.5 py-1.5 border rounded-[var(--r-control)] text-[length:var(--text-sm)] uppercase tracking-widest transition-colors',
                  freeLabel === area
                    ? 'border-[color:var(--violet-400)] text-[color:var(--ink)] bg-white/[0.05]'
                    : 'border-[color:var(--hairline)] text-[color:var(--ink-faint)] hover:border-[color:var(--border-strong)]'
                )}
